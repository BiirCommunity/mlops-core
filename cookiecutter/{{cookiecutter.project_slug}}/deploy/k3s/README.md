...
Образы **собираются** (`docker build`) и **пушатся** в registry на `{{ cookiecutter.mlops_registry }}`. k3s **pull** при старте подов. Перенос через `docker save | ctr import` не используется.
...
Другой registry: `MLOPS_REGISTRY={{ cookiecutter.lan_ip }}:5000 ./scripts/k3s-build-images.sh` и обновите `images:` в `k8s/kustomization.yaml`.
